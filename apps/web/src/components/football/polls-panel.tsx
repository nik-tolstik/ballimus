...
function PollNotificationSettingsEditor({ poll, saving, onSave, onCancel, onRepublish, onArchive }: { readonly poll: PollResponseDto; readonly saving: boolean; readonly onSave: (notificationEnabled: readonly boolean[]) => void; readonly onCancel: () => void; readonly onRepublish: () => void; readonly onArchive: () => void }) {
  ...
  return <form className="min-h-0 flex flex-1 flex-col" onSubmit={(event) => { event.preventDefault(); onSave(notificationEnabled) }}>
    ...
    <div className="flex gap-2 border-t bg-background p-4 pb-[max(1rem,calc(1rem+var(--tg-safe-bottom)))]"><Button type="button" variant="ghost" className="h-11 flex-1 border" disabled={saving} onClick={onCancel}>Отмена</Button><Button type="submit" className="h-11 flex-1" disabled={saving}>{saving ? <LoaderCircle className="animate-spin" data-icon="inline-start" /> : <Check data-icon="inline-start" />}Сохранить</Button></div>
  </form>
}
