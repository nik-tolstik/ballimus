...
import { Ellipsis, Plus, Save, Trash2 } from 'lucide-react'
...
export function VenueForm({ venue, onSave, onOpenActions, saving }: VenueFormProps) {
  ...
  return <form aria-label={venue === undefined ? 'Форма создания места' : 'Форма редактирования места'} className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => { event.preventDefault(); submit() }}>
    <FieldGroup className="min-h-0 flex-1 gap-4 overflow-y-auto px-4 pb-5">
      ...
      <FieldGroup className="gap-2"><FieldLabel>Контакты для бронирования</FieldLabel>{contacts.map((contact, index) => <div key={index} className={`grid gap-2 ${contacts.length > 1 ? 'grid-cols-[minmax(0,1fr)_7.5rem_2.5rem]' : 'grid-cols-[minmax(0,1fr)_7.5rem]'}`}><Input aria-label={`Телефон для бронирования ${index + 1}`} type="tel" inputMode="tel" maxLength={50} value={contact.phone} onChange={(event) => setContacts((current) => current.map((value, itemIndex) => itemIndex === index ? { ...value, phone: event.target.value } : value))} placeholder="+375 …" /><Input aria-label={`Имя контакта ${index + 1}`} maxLength={100} value={contact.name} onChange={(event) => setContacts((current) => current.map((value, itemIndex) => itemIndex === index ? { ...value, name: event.target.value } : value))} placeholder="Имя" />{contacts.length > 1 ? <Button type="button" variant="ghost" size="icon" className="size-10" aria-label={`Удалить контакт ${index + 1}`} onClick={() => setContacts((current) => current.filter((_value, itemIndex) => itemIndex !== index))}><Trash2 /></Button> : null}</div>)}<Button type="button" variant="ghost" disabled={contacts.length >= 5} onClick={() => setContacts((current) => [...current, { name: '', phone: '' }])}><Plus data-icon="inline-start" />Добавить контакт</Button></FieldGroup>
    ...
    <div className="sheet-actions bg-muted/60 px-4 pt-3 pb-[max(1rem,calc(1rem+var(--tg-safe-bottom)))]"><div className="flex gap-2"><Button type="submit" className="h-10 flex-1" disabled={saving}><Save data-icon="inline-start" />{saving ? 'Сохранение…' : venue === undefined ? 'Добавить' : 'Сохранить'}</Button>{venue !== undefined && onOpenActions !== undefined ? <Button type="button" variant="ghost" size="icon-lg" className="size-10" aria-label="Действия места" disabled={saving} onClick={onOpenActions}><Ellipsis /></Button> : null}</div></div>
  </form>
}
