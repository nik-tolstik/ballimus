...
export function VenueForm({ venue, onSave, saving }: VenueFormProps) {
  ...
  const [phones, setPhones] = useState<string[]>(venue?.bookingPhones.length ? [...venue.bookingPhones] : [''])
  ...
  const submit = () => {
    ...
    if (normalizedPhones.some((phone) => !phonePattern.test(phone))) { setError('Проверьте номер телефона для бронирования.'); return }
    if (new Set(normalizedPhones).size !== normalizedPhones.length) { setError('Номера телефонов не должны повторяться.'); return }
    setError('')
    void onSave({ name: normalizedName, mapUrl: normalizedMapUrl, venueType, bookingPhones: normalizedPhones, websiteUrl: normalizedWebsiteUrl === '' ? null : normalizedWebsiteUrl })
  }
  ...
  return <form aria-label={venue === undefined ? 'Форма создания места' : 'Форма редактирования места'} className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => { event.preventDefault(); submit() }}>
    <FieldGroup className="min-h-0 flex-1 gap-4 overflow-y-auto px-4 pb-5">
      ...
      <FieldGroup className="gap-2"><FieldLabel>Телефоны для бронирования</FieldLabel>{phones.map((phone, index) => <div key={index} className="grid grid-cols-[minmax(0,1fr)_2.5rem] gap-2"><Input aria-label={`Телефон для бронирования ${index + 1}`} type="tel" inputMode="tel" value={phone} onChange={(event) => setPhones((current) => current.map((value, itemIndex) => itemIndex === index ? event.target.value : value))} placeholder="+375 …" />{phones.length > 1 ? <Button type="button" variant="ghost" size="icon" className="size-10" aria-label={`Удалить телефон ${index + 1}`} onClick={() => setPhones((current) => current.filter((_value, itemIndex) => itemIndex !== index))}><Trash2 /></Button> : <span />}</div>)}<Button type="button" variant="outline" disabled={phones.length >= 5} onClick={() => setPhones((current) => [...current, ''])}><Plus data-icon="inline-start" />Добавить номер</Button></FieldGroup>
