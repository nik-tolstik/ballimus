...
export function VenuesPanel({ venues, onCreate, onUpdate, onDelete, saving }: VenuesPanelProps) {
  ...
  const deleteVenue = async (venue: NormalizedVenue) => {
    if (await onDelete(venue)) { setConfirmationOpen(false); setSheetOpen(false) }
  }
  const closeConfirmation = (open: boolean) => { setConfirmationOpen(open); if (!open) setActionsOpen(true) }
