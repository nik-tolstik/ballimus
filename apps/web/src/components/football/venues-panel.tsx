...
export function VenuesPanel({ venues, onCreate, onUpdate, onArchive, onRestore, saving }: VenuesPanelProps) {
  ...
  return <section className="flex flex-col gap-5">
    ...
    <Sheet open={sheetOpen} onOpenChange={setSheetOpen}><SheetContent side="bottom" className="mx-auto max-h-[92svh] w-full max-w-[480px] gap-0 rounded-t-2xl p-0"><div className="mx-auto mt-2 h-1 w-10 rounded-full bg-muted-foreground/35" /><SheetHeader className="px-4 pt-3 pb-4"><SheetTitle className="text-lg">{editing === undefined ? 'Новое место' : 'Редактировать место'}</SheetTitle></SheetHeader><VenueForm key={editing?.id ?? 'new'} {...(editing === undefined ? {} : { venue: editing })} saving={saving} onSave={async (values) => { if (editing === undefined) await onCreate(values); else await onUpdate(editing, values); setSheetOpen(false) }} {...(editing === undefined ? {} : { onArchiveStateChange: () => changeArchiveState(editing) })} /></SheetContent></Sheet>
  </section>
}
