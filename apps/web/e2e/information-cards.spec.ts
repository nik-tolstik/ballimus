...
test('creates a native poll with an option threshold notification', async ({ page }) => {
  ...
  await expect(page.getByLabel('Вариант 2', { exact: true })).toHaveCount(0)
  ...
  await expect(page.getByLabel('Вариант 2', { exact: true })).toBeVisible()
  await expect(publishPoll).toBeDisabled()
  await page.getByLabel('Вариант 2', { exact: true }).fill('Не буду')
  await expect(page.getByLabel('Вариант 3', { exact: true })).toBeVisible()
  ...
  await expect(notification).toBeChecked()
  await expect(secondOptionNotification).toHaveAttribute('aria-pressed', 'false')
  await page.getByLabel('Вариант 3', { exact: true }).fill('Временно')
  await expect(page.getByLabel('Вариант 4', { exact: true })).toBeVisible()
  ...
  await expect(page.getByLabel('Вариант 4', { exact: true })).toHaveCount(0)
