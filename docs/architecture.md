...
- native poll creation, active and archived listing, notification settings, archiving, and permanent deletion from the archive;
...
New polls are non-anonymous and target the configured General topic. Creation performs one bounded Bot API call and records `published`, `failed`, or `uncertain` immediately; it does not queue automatic or manual publication retries. Failed and uncertain polls remain visible with their status and can be archived after the owner checks General. Telegram `poll` updates refresh ordered option counts, and the visible Polls screen periodically reloads those persisted counts. A poll can have one notification threshold, and every option stores a direct notification-enabled flag. After creation, the owner may change only those ordered option flags; the question, option text and order, threshold, Telegram references, and native poll remain immutable. The edit and webhook paths lock the same poll row, so a count update cannot overwrite notification settings. Each upward crossing of the threshold on an enabled option atomically marks that crossing and makes one bounded send to the configured Chat topic; falling below the threshold rearms the option for a later crossing. Enabling an option that is already above the threshold does not send a message until a later fresh upward crossing. Individual voter answers detect downward crossings, wait for a 10-second grace period, and recheck both the current total and the voter's latest answer before making one direct alert attempt. Disabling an option cancels its pending delayed withdrawal check and suppresses both transitions. Failures are not retried. Archiving a poll hides it immediately, suppresses threshold work, prevents notification edits, and makes one bounded attempt to delete its Telegram message. Archived polls are listed by archival time and can then be permanently deleted without another Telegram deletion attempt. Polls and notifications never reference a match or use the durable outbox.
