...
A poll has a question, 2–12 ordered options, and an optional multiple-answer setting. The editor starts with one draft option and automatically adds another blank row when text is entered in the last option. Clearing a populated option removes its row and moves focus to an adjacent option; at least two populated options are required to publish. Polls are anonymous regular polls and explicitly allow voters to change or retract their choices. The bell beside an option directly enables or disables count notifications for that option; new options have the bell enabled by default.
