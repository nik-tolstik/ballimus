...
5. Keep the check read-only unless the owner separately authorizes a specific mutation. Navigation, opening sheets, cancelling UI confirmations, temporary unsaved changes, console inspection, and screenshots are safe. Creating, saving, archiving, permanently deleting archived records, voting, or sending weather can change local data or send Telegram messages and must target only the local bot, local database, and `Футбол тест` group.
...
2. Edit the match and verify that the same message changes, then archive it.
3. Send current Minsk weather and verify that the message appears in the configured test topic.
4. Create a non-anonymous native poll with one option notification enabled. Verify publication, live counts, threshold crossings, delayed shortage behavior, the large **Save** action for existing poll notifications, archival, and permanent deletion from the poll archive. Verify that failed and uncertain poll statuses do not expose a republish action. Disable a bell during the 10-second withdrawal grace period and verify that no alert is sent. Re-enable a bell above the threshold and verify that no immediate message is sent; the next upward crossing should notify.
